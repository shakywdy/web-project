 ...
 * @LastEditTime: 2023-10-27 17:04:29
 * @FilePath: /web-project/data/date-control.php
 * Intimat: jason
...
if (isset($_POST['userid']) && isset($_POST['username']) && isset($_POST['content'])) {
    ...
    if (isset($_POST['fileName']) && isset($_POST['fileType']) && isset($_POST['fileSize'])) {
        ...
        $counter = 1; 
        while (true) {
            $sql2 = "SELECT link FROM discussion WHERE link = '$link'";
            $result = mysqli_query($db, $sql2);
        
            if ($result && mysqli_num_rows($result) > 0) {
                $link = $filename . '(' . $counter . ')' . '.' . $filetype;
                $counter++;
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    while (file_exists($targetFile)) {
        ...
        $newFileName = pathinfo($originalFileName, PATHINFO_FILENAME) . "({$counter})" . '.' . $fileExtension;
